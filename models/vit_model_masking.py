...
class VisionTransformerWithMasking(nn.Module):
    def __init__(self, patch_size, embedding_dim, num_heads, num_layers, num_classes, device, mask_ratio=0.15):
        super(VisionTransformerWithMasking, self).__init__()
        ...
        # Assume a specific fixed input image size
    ...
    def forward(self, x):
        ...
        # Calculate the number of patches dynamically based on the input dimensions
        ...
        if self.training:
            ...
            mask = torch.ones(B, num_patches + 1, self.embedding_dim, device=x.device)
            mask[:, mask_indices + 1, :] = 0  # +1 to account for the cls_token position
            x = x * mask
